...
AudioManager::AudioManager()
  : CThread(CThread::eAttributeAffCore1 | CThread::eAttributePinnedAff, 16, 0x80000)
  , m_voices{0}

{
    s_instance = this;

    LOG(LogAudio, "Starting up AudioManager");

    OSInitMessageQueueEx(&m_frameCbQueue, m_frameCbMsg, 8, "AudioManager::m_frameCbQueue");

    OSInitMutexEx(&m_acquireVoiceMutex, "AudioManager::m_acquireVoiceMutex");
...
AudioManager::~AudioManager()
{
    LOG(LogAudio, "Shutting down AudioManager");
...
int AudioManager::AcquireVoice(u32 volume, bool tvLeft, bool tvRight, bool drcLeft, bool drcRight,
  u32 sampleRate, bool streamed, bool looped)
{
    if (m_shutdown)
        return -1;
    ...
    for (u32 i = 0; i < m_voices.size(); i++) {
        if (!m_voices[i].m_inUse && m_voices[i].m_voice == nullptr) {
            ...
            OSInitMessageQueueEx(&m_voices[i].m_bufferInQueue, m_voices[i].m_bufferInMsg, 8,
              "AudioManager::Voice::m_bufferInQueue");

            OSInitMessageQueueEx(&m_voices[i].m_ctrlQueue, m_voices[i].m_ctrlMsg, 8,
              "AudioManager::Voice::m_ctrlQueue");

            OSInitMessageQueueEx(&m_voices[i].m_ctrlRespQueue, m_voices[i].m_ctrlRespMsg, 8,
              "AudioManager::Voice::m_ctrlRespQueue");
            ...
            auto ret = OSSendMessage(&m_voices[i].m_ctrlQueue, &msg, OS_MESSAGE_FLAGS_BLOCKING);
            assert(ret == TRUE);
            return i;
        }
    }
...
void AudioManager::FreeVoice(int voice)
{
    if (m_shutdown)
        return;
    ...
    auto ret = OSSendMessage(&m_voices[voice].m_ctrlQueue, &msg, OS_MESSAGE_FLAGS_BLOCKING);
    assert(ret == TRUE);

    ret = OSReceiveMessage(&m_voices[voice].m_ctrlRespQueue, &msg, OS_MESSAGE_FLAGS_BLOCKING);
    assert(ret == TRUE);

    m_voices[voice] = {};
}

void AudioManager::PushBuffer(int voice, u16* buffer, u32 sampleCount, u32 marker)
{
    if (m_shutdown)
        return;

    if (!m_voices[voice].m_inUse) {
        LOG(LogAudio, "Attempt to push buffer to a voice that's not in use (%d)", voice);
        return;
    }
    ...
    auto ret = OSSendMessage(&m_voices[voice].m_bufferInQueue, &msg, OS_MESSAGE_FLAGS_BLOCKING);
    assert(ret == TRUE);
}

void AudioManager::SetBufferOutQueue(int voice, OSMessageQueue* queue)
{
    if (m_shutdown)
        return;
...
void AudioManager::Start(int voice)
{
    if (m_shutdown)
        return;
...
void AudioManager::Restart(int voice)
{
    if (m_shutdown)
        return;

    if (!m_voices[voice].m_inUse) {
        LOG(LogAudio, "Attempt to restart a voice that's not in use (%d)", voice);
        return;
    }
...
void AudioManager::ChangeMarker(int voice, u32 marker)
{
    if (m_shutdown)
        return;
...
void AudioManager::InternalInitVoice(
...
void AudioManager::InternalVoiceSetOffsets(Voice* vc, bool loop)
{
    assert(vc != nullptr);
    ...
    vc->m_voiceOffsets = {
      ...
      .loopingEnabled = loop && (vc->m_nextBuffer || !vc->m_streamed) ? AX_VOICE_LOOP_ENABLED
                                                                      : AX_VOICE_LOOP_DISABLED,
      .loopOffset = loop && vc->m_nextBuffer ? vc->m_nextBuffer - vc->m_buffer : 0,
    ...
    vc->m_voiceSrc = {};
    vc->m_voiceSrc.ratio = u32(0x00010000 * (float(vc->m_sampleRate) / float(samplesPerSec)));
...
void AudioManager::executeThread()
{
    if (AXIsInit() == TRUE) {
    ...
    while (true) {
        ...
        OSMessage msg;
        if (!OSReceiveMessage(&m_frameCbQueue, &msg, OS_MESSAGE_FLAGS_BLOCKING)) {
        ...
        for (u32 i = 0; i < m_voices.size(); i++) {
            ...
            auto sendBufferBack = [&](void* sbbBuffer, u32 sbbSampleCount) {
                ...
                auto ret = OSSendMessage(vc->m_bufferOutQueue, &msg, OS_MESSAGE_FLAGS_BLOCKING);
                assert(ret == TRUE);
            };
            ...
            if (vc->m_inUse && vc->m_voice != nullptr) {
                ...
                if (vc->m_streamed && vc->m_lastLoopCount != loopCount) {
                    ...
                    if (vc->m_nextBuffer != nullptr) {
                        ...
                        AXSetVoiceEndOffset(vc->m_voice,
                          vc->m_voiceOffsets.loopOffset + vc->m_nextBufferSampleCount - 1);
                ...
                if (vc->m_streamed && vc->m_buffer == nullptr && vc->m_nextBuffer != nullptr) {
                    // Check if marker is less than current one and immediately
                    // send it back.
                    if (vc->m_marker > vc->m_nextBufferMarker) {
                        sendBufferBack(vc->m_nextBuffer, vc->m_nextBufferSampleCount);
                        vc->m_nextBuffer = nullptr;
                ...
                if (!vc->m_streamed || (vc->m_streamed && vc->m_nextBuffer == nullptr)) {
                    if (OSReceiveMessage(&vc->m_bufferInQueue, &msg, OS_MESSAGE_FLAGS_NONE)) {
                        ...
                        if (msg.args[1] < vc->m_marker) {
                            ...
                            if ((vc->m_buffer == nullptr || !vc->m_streamed) &&
                                ...
                                if (vc->m_state == Voice::State::WaitingOnMarker ||
                                    (vc->m_state == Voice::State::Playing &&
                                      vc->m_voice->state == AX_VOICE_STATE_STOPPED)) {
                                    vc->m_state = Voice::State::Playing;
                                    AXSetVoiceState(vc->m_voice, AX_VOICE_STATE_PLAYING);
                                }
                            } else {
                                ...
                                vc->m_voiceOffsets.loopOffset =
                                  vc->m_nextBuffer - (u16*) vc->m_voiceOffsets.data;
                                if (vc->m_nextBufferMarker != vc->m_marker) {
                                    AXSetVoiceLoop(vc->m_voice, AX_VOICE_LOOP_DISABLED);
                                } else {
                                    AXSetVoiceLoop(vc->m_voice, AX_VOICE_LOOP_ENABLED);
            ...
            if (OSReceiveMessage(&vc->m_ctrlQueue, &msg, OS_MESSAGE_FLAGS_NONE)) {
                switch (msg.args[0]) {
                case Voice::MSG_INIT:
                    vc->m_state = Voice::State::Stopped;
                    InternalInitVoice(i, vc->m_initArgs.volume, vc->m_initArgs.tvLeft,
                      vc->m_initArgs.tvRight, vc->m_initArgs.drcLeft, vc->m_initArgs.drcRight);
                    break;
                ...
                case Voice::MSG_FREE: {
                    ...
                    while (OSReceiveMessage(&vc->m_bufferInQueue, &msg, OS_MESSAGE_FLAGS_NONE)) {
                        if (vc->m_bufferOutQueue != nullptr) {
                            OSSendMessage(vc->m_bufferOutQueue, &msg, OS_MESSAGE_FLAGS_BLOCKING);
                        } else {
                            free(msg.message);
                        }
                    }
                    ...
                    msg = {};
                    auto ret = OSSendMessage(&vc->m_ctrlRespQueue, &msg, OS_MESSAGE_FLAGS_BLOCKING);
                    assert(ret);
                    break;
                }
                ...
                case Voice::MSG_CHANGE_MARKER: {
                    ...
                    if (vc->m_nextBuffer != nullptr && vc->m_nextBufferMarker < newMarker) {
                        sendBufferBack(vc->m_nextBuffer, vc->m_nextBufferSampleCount);

                        vc->m_nextBuffer = nullptr;

                        while (
                          OSReceiveMessage(&vc->m_bufferInQueue, &msg, OS_MESSAGE_FLAGS_NONE)) {
                            assert(msg.message != nullptr);
...
void AudioManager::AXFrameCallbackFunc()
{
    OSMessage msg = {
      .message = nullptr,
      .args = {MSG_AXFRAME},
    ...
    auto obj = AudioManager::s_instance;
    assert(obj != nullptr);

    auto ret = OSSendMessage(&obj->m_frameCbQueue, &msg, OS_MESSAGE_FLAGS_BLOCKING);
    assert(ret == TRUE);
}
