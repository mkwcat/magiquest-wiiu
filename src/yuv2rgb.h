...
#define FIXED_POINT_VALUE(value, precision)                                    \
  ((int) (((value) * (1 << precision)) + 0.5))
...
#define YUV2RGB_PARAM(Rf, Bf, YMin, YMax, CbCrRange)                           \
  {                                                                            \
    ...
    .g_cb_factor = FIXED_POINT_VALUE(                                          \
      Bf / (1.0 - Bf - Rf) * 255.0 * (2.0 * (1 - Bf)) / CbCrRange, 7),         \
    .g_cr_factor = FIXED_POINT_VALUE(                                          \
      Rf / (1.0 - Bf - Rf) * 255.0 * (2.0 * (1 - Rf)) / CbCrRange, 7),         \
    .y_factor = FIXED_POINT_VALUE(255.0 / (YMax - YMin), 7), .y_offset = YMin  \
  }
...
static inline void nv12_rgba32_std(uint32_t width, uint32_t height,
  const uint8_t* Y, const uint8_t* UV, uint32_t Y_stride, uint32_t UV_stride,
  uint8_t* RGB __restrict, uint32_t RGB_stride, int yuv_type)
{
    ...
    for (y = 0; y < (height - 1); y += 2) {
        ...
        u32* rgb_ptr1 __restrict = (u32*) (RGB + y * RGB_stride);
        u32* rgb_ptr2 __restrict = (u32*) (RGB + (y + 1) * RGB_stride);
        ...
        for (x = 0; x < (width - 1); x += 2) {
            ...
            g_cbcr_offset =
              (param->g_cb_factor * u_tmp + param->g_cr_factor * v_tmp) >> 7;

            int16_t y_tmp =
              (param->y_factor * (y_ptr1[0] - param->y_offset)) >> 7;
            rgb_ptr1[0] = 0xFF | (clamp(y_tmp + r_cr_offset) << 24) |
                          (clamp(y_tmp - g_cbcr_offset) << 16) |
                          (clamp(y_tmp + b_cb_offset) << 8);
